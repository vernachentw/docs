...
為滿足電子圍籬跨系統應用需求，GIPSense 可針對電子圍籬事件發生時，將事件訊息推播至外部系統，由外部應用系統來設計處理事件對應之邏輯，如介接蜂鳴器告警、顯示資訊在門禁系統等，本文件說明介接之方式。
...
由接收端提供 POST API URL，GIPSesne 於事件發生時，發送事件推播至該 URL，並以標準事件 JSON 訊息格式提供
